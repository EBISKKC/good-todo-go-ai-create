...
import (
	...
	"errors"
	"fmt"
	"good-todo-go/internal/ent/generated/predicate"
...
func (_u *TenantUpdate) AddTodoIDs(ids ...string) *TenantUpdate {
	_u.mutation.AddTodoIDs(ids...)
	return _u
}

// AddTodos adds the "todos" edges to the Todo entity.
func (_u *TenantUpdate) AddTodos(v ...*Todo) *TenantUpdate {
	ids := make([]string, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *TenantUpdate) check() error {
	...
	if v, ok := _u.mutation.Slug(); ok {
		if err := tenant.SlugValidator(v); err != nil {
			return &ValidationError{Name: "slug", err: fmt.Errorf(`generated: validator failed for field "Tenant.slug": %w`, err)}
		}
	}
	return nil
}
...
func (_u *TenantUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.TodosCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   tenant.TodosTable,
			Columns: []string{tenant.TodosColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
...
// AddTodoIDs adds the "todos" edge to the Todo entity by IDs.
func (_u *TenantUpdateOne) AddTodoIDs(ids ...string) *TenantUpdateOne {
	_u.mutation.AddTodoIDs(ids...)
	return _u
}

// AddTodos adds the "todos" edges to the Todo entity.
func (_u *TenantUpdateOne) AddTodos(v ...*Todo) *TenantUpdateOne {
	ids := make([]string, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// ClearTodos clears all "todos" edges to the Todo entity.
func (_u *TenantUpdateOne) ClearTodos() *TenantUpdateOne {
	_u.mutation.ClearTodos()
	return _u
}

// RemoveTodoIDs removes the "todos" edge to Todo entities by IDs.
func (_u *TenantUpdateOne) RemoveTodoIDs(ids ...string) *TenantUpdateOne {
	_u.mutation.RemoveTodoIDs(ids...)
	return _u
}

// RemoveTodos removes "todos" edges to Todo entities.
func (_u *TenantUpdateOne) RemoveTodos(v ...*Todo) *TenantUpdateOne {
	ids := make([]string, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
